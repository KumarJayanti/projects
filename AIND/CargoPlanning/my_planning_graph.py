...
#logging.basicConfig(filename='MyPlanningGraph.log',level=logging.DEBUG)
...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        # TODO assuming is_pos is true for all goal set nodes
        # this is used for efficient h_levelsum() implementation
        self.goal_set_nodes = set(PgNode_s(a, True) for a in self.problem.goal)
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            #check if this proposed PgNode_a has prenodes that are a subset 
            #of the previous S level.
            if a_node.prenodes.issubset(self.s_levels[level]):
                #add the action for this level
                self.a_levels[level].add(a_node)
                for s_node in self.s_levels[level]:
                    #connect to S node from previous S literal level
    ...
    def add_literal_level(self, level):
        ...
        #start with empty literal set
        self.s_levels.append(set())
        
        for a_node in self.a_levels[level - 1]:
            for s_node in a_node.effnodes:
                # each A node in the previous level has a list of 
                # S nodes in effnodes that represent the effect
                # produced by the action. 
                
                # At this S level we could either reference the effnodes(PgNode_s)
                # present in the PgNode_a at level-1 or create fresh 
                # S nodes. I would prefer creating a fresh clone
                s_node_new = PgNode_s(s_node.symbol, s_node.is_pos)
                self.s_levels[level].add(s_node_new)
                #retrieve the new s_node_new reference from the set again before adding to
                # its parent and child sets
                for s_node_new in self.s_levels[level]:
                    # add the A node to the parent set of the S node
                    s_node_new.parents.add(a_node)
                    #connect the new S node as child of the A node
                    a_node.children.add(s_node_new)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        assert node_a1 != None, "node_a1 is None"
        assert node_a2 != None, "node_a2 is None"
        
        #test for Inconsistent Effects between nodes
        action1 = node_a1.action
        action2 = node_a2.action
        # if action1's add effects are cancelled by action2's rem effects
        for effect in action1.effect_add:
            if effect in action2.effect_rem:
                return True

        # if action2's add effects are cancelled by action1's rem effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1 = node_a1.action
        action2 = node_a2.action
        # if action1's add effect is negation of action2's precond or vice-versa
        # Ex  Eat(Cake) -> add effect = Eaten(Cake)
        # Persistence(~Eaten(Cake)) -> precond_neg = Eaten(Cake)
        for effect in action1.effect_add:
            if effect in action2.precond_neg:
                return True
        for effect in action2.effect_add:
            if effect in action1.precond_neg:
                return True
        # if action1's rem effect is negation of action2's precond or vice-versa
        # Ex Eat(Cake) - rem effect = Have(Cake)
        # Persistence(Have(Cake)) - precond_pos = Have(Cake)
        for effect in action1.effect_rem:
            if effect in action2.precond_pos:
                return True
        for effect in action2.effect_rem:
            if effect in action1.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #ex Bake(Cake) and Eat(Cake) are mutex because they compete on the value 
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return node_s1 == PgNode_s(node_s2.symbol, not node_s2.is_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for node_a1 in node_s1.parents:
            for node_a2 in node_s2.parents:
                #check for pairwise mutex of all actions that could achieve 
                #first literal with all actions that could achieve second literal
    ...
    def h_levelsum(self) -> int:
        ...
        goal_subset_prev = set()
        goal_subset = set()
        for level in range(len(self.s_levels)):
            state = self.s_levels[level]
            #intersect to find which goals are found in this level
            goal_subset = self.goal_set_nodes & state
            level_sum += level * len(goal_subset - goal_subset_prev)
            goal_subset_prev = goal_subset    
        return level_sum
